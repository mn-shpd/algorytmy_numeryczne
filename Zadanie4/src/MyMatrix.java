...
public class MyMatrix {
    ...
    public MyMatrix subtract(MyMatrix secondMatrix) {
...
//                this.set(i, j, (double) randomValue / 65536);
    ...
    public MyMatrix jacobIteration(int numberOfIterations) throws Exception {

        //Macierz wynikowa X.
        MyMatrix X;

        if(rowCount != columnCount-1) {
            throw new Exception("Not enough rows or columns.");
        } else {

            //this.removeZerosFromDiagonal();

            //Macierz X poprzedniej iteracji.
            X = new MyMatrix(rowCount, 1);
            X.setZero();

            //Macierz X danej iteracji.
            MyMatrix newX = new MyMatrix(rowCount, 1);

            for(int k = 1; k <= numberOfIterations; k++) {
                for(int i = 0; i < rowCount; i++) {
                    //Wspolczynnik b.
                    double temp = this.getValue(i, columnCount - 1);

                    for(int j = 0; j < columnCount - 1; j++) {
                        if(j != i) {
                            //Odjecie pozostalych wspolczynnikow A wymnozonczych przez X-ksy z poprzedniej iteracji.
                            temp -= (this.getValue(i, j) * X.getValue(j, 0));
                        }
                    }
                    //Podzielenie przez wspolczynnik (i,i) i zapisanie wartosci x.
                    newX.set(i, 0, temp / this.getValue(i, i));
                }
                X = newX.copyXMatrix();
            }
        }
    ...
    public MyMatrix gaussSeidelIteration(int numberOfIterations) throws Exception {
        ...
        if(rowCount != columnCount-1) {
            ...
            for(int k = 1; k <= numberOfIterations; k++) {
                ...
                for(int i = 0; i < rowCount; i++) {
                    ...
                    //Podzielenie przez wspolczynnik (i,i) i zapisanie wartosci x.
                    X.set(i, 0, temp / this.getValue(i, i));
                }
            }
        }
        return X;
    ...
    public MyMatrix copyXMatrix() {

        MyMatrix newMatrix = new MyMatrix(rowCount, 1);

        for(int i = 0; i < rowCount; i++) {
            newMatrix.set(i, 0, this.matrix[i][0]);
        }

        return newMatrix;
    }

    //Likwiduje zera z przekatnej, przestawiajac wiersze.
    public void removeZerosFromDiagonal() throws Exception {

        if(rowCount != columnCount - 1) {
            throw new Exception("This is not a square matrix. It does not have a diagonal.");
        }

        for(int i = 0; i < rowCount; i++) {
            if(getValue(i, i) == 0.0) {
                for(int k = 0; k < rowCount; k++) {
                    if((k != i) && getValue(k, i) != 0.0) {
                        swapRows(i, k);
                        removeZerosFromDiagonal();
                    }
                }
            }
        }
    ...
    public MyMatrix invertValuesOnDiagonal() {
    ...
    //Zwraca wspolczynniki macierzy jako liste
    public List<Double> getMatrixAsList() {
